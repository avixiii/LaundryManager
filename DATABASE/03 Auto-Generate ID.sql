...
-- TỰ ĐỘNG TẠO MÃ THU CHI
CREATE FUNCTION fcgetCashBookID()
RETURNS VARCHAR(15)
AS
BEGIN
	DECLARE @Idx INT = 1
	DECLARE @ID VARCHAR(15) = 'TC00.0001'

	WHILE EXISTS ( SELECT ID FROM dbo.CashBook WHERE ID = @ID)
	BEGIN
		SET @Idx = @Idx + 1
		SET @ID = 'TC00.' + REPLICATE('0', 4 - LEN(CAST(@Idx AS VARCHAR))) + CAST(@Idx AS VARCHAR)
	END
	
	RETURN CONVERT(VARCHAR(10), @ID)
